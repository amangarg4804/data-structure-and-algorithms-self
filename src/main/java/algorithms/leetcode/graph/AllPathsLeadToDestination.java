...
//            currentPath.add(neighbour);
    ...
    public boolean leadsToDestination2(int n, int[][] edges, int source, int destination) {
        ...
        // How to implement? instead of primitive boolean visited array, which can store only true false. we can have a wrapper Boolean array storing whether the node leads to destination node
        // null value- not visited
        // true- leads to destination
        // false
        //  - doesn't lead to destination
        //  - or all child nodes of current node are not yet processed. This will help in detecting cycle when we visit this already visited node again
        Map<Integer, List<Integer>> graph = new HashMap<>();
        for (int[] edge : edges) {
            int start = edge[0];
            int end = edge[1];
        ...
        // result[i] denotes whether the node i leads to destination
